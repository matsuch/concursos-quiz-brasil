-- Drop the problematic view that exposes auth.users
DROP VIEW IF EXISTS public.user_subscription_details;

-- Recreate the view without joining auth.users
-- Instead, we'll add email to the subscriptions table or fetch it separately
CREATE OR REPLACE VIEW public.user_subscription_details AS
SELECT 
    s.id,
    s.user_id,
    s.stripe_customer_id,
    s.stripe_subscription_id,
    s.stripe_price_id,
    s.status,
    s.plan_name,
    s.plan_amount,
    s.currency,
    s.current_period_start,
    s.current_period_end,
    s.cancel_at_period_end,
    s.canceled_at,
    s.trial_end,
    s.created_at,
    s.updated_at,
    NULL::text AS email,
    CASE
        WHEN ((s.status = 'active'::text) AND (s.cancel_at_period_end = true)) THEN 'Cancelando'::text
        WHEN (s.status = 'active'::text) THEN 'Ativa'::text
        WHEN (s.status = 'trialing'::text) THEN 'Período de Teste'::text
        WHEN (s.status = 'canceled'::text) THEN 'Cancelada'::text
        WHEN (s.status = 'past_due'::text) THEN 'Pagamento Atrasado'::text
        ELSE 'Inativa'::text
    END AS status_display
FROM subscriptions s;

-- Also fix the profiles_ranking view to include display_name
DROP VIEW IF EXISTS public.profiles_ranking;

CREATE OR REPLACE VIEW public.profiles_ranking AS
SELECT 
    user_id,
    display_name,
    total_points,
    quizzes_completed
FROM profiles;
