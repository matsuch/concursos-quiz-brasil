        ...
        <div className="flex gap-2 flex-wrap">
          {cycles.map((cycle) => (
            <Badge
              ...
              className="cursor-pointer py-1.5 px-3 rounded-lg transition-all hover:scale-105 text-xs"
              onClick={() => setSelectedCycle(cycle)}
            >
              <RotateCcw className="w-3 h-3 mr-1.5" />
              <span className="truncate max-w-[120px]">{cycle.name} ({cycle.duration_days}d)</span>
            </Badge>
          ))}
        </div>
      )}
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div className="space-y-1">
                <CardTitle className="flex items-center gap-2 text-lg">
                  ...
                  <span className="truncate">{selectedCycle?.name || activeCycle.name}</span>
                </CardTitle>
              ...
              <Dialog open={isAddingBlock} onOpenChange={setIsAddingBlock}>
                <DialogTrigger asChild>
                  <Button size="sm" variant="outline" className="rounded-lg text-sm px-3 py-2 h-auto">
                    <Plus className="w-4 h-4 mr-2" />
                      ...
                      <div className="flex gap-2 flex-wrap mt-2">
                        {SUBJECT_COLORS.map((color) => (
                          <button
                            key={color.value}
                            type="button"
                            className={`w-8 h-8 rounded-full border-2 ${newBlockColor === color.value ? 'border-foreground' : 'border-transparent'} transition-transform hover:scale-110`}
                ...
                {cycleBlocks.map((block, index) => (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      <span className="flex items-center justify-center w-7 h-7 rounded-lg bg-muted text-xs font-semibold text-muted-foreground flex-shrink-0">
                        {index + 1}
                      </span>
                      <div className="min-w-0 flex-1">
                        <p className="font-medium text-sm text-foreground truncate">{block.subject}</p>
