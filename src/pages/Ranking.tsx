          ...
          <div className="grid grid-cols-3 gap-4 mb-12">
            ...
            <div className="pt-8">
              <Card className="p-6 text-center border-2 border-border">
                ...
                <Avatar className="w-16 h-16 mx-auto mb-3 bg-secondary/10">
                  <AvatarFallback className="text-secondary font-bold">
                    {mockRanking[1].name.split(' ').map(n => n[0]).join('')}
                  </AvatarFallback>
                </Avatar>
                <p className="font-semibold mb-1">{mockRanking[1].name}</p>
                <p className="text-2xl font-bold text-secondary mb-1">{mockRanking[1].points}</p>
            ...
            {/* 1st Place */}
            <div className="pt-0">
              <Card className="p-6 text-center border-2 border-primary bg-gradient-to-br from-primary/5 to-transparent">
                <div className="flex justify-center mb-3">
                  {getPositionIcon(1)}
                </div>
                <Avatar className="w-20 h-20 mx-auto mb-3 bg-primary/10 border-4 border-primary">
                  <AvatarFallback className="text-primary font-bold text-xl">
                    {mockRanking[0].name.split(' ').map(n => n[0]).join('')}
                  </AvatarFallback>
                </Avatar>
                <p className="font-semibold mb-1">{mockRanking[0].name}</p>
                <p className="text-3xl font-bold text-primary mb-1">{mockRanking[0].points}</p>
                <p className="text-xs text-muted-foreground">pontos</p>
              </Card>
            </div>
            ...
            <div className="pt-8">
              <Card className="p-6 text-center border-2 border-border">
                ...
                <Avatar className="w-16 h-16 mx-auto mb-3 bg-accent/10">
                  <AvatarFallback className="text-accent-foreground font-bold">
                    {mockRanking[2].name.split(' ').map(n => n[0]).join('')}
                  </AvatarFallback>
                </Avatar>
                <p className="font-semibold mb-1">{mockRanking[2].name}</p>
                <p className="text-2xl font-bold text-accent-foreground mb-1">{mockRanking[2].points}</p>
                <p className="text-xs text-muted-foreground">pontos</p>
              </Card>
            </div>
          </div>
